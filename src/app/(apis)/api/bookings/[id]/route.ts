...
export async function GET(
	...
	try {
		...
		try {
			if (
				process.env.ENABLE_INVOICES_DUAL_WRITE === 'true' &&
				bills?.[0]?.id
			) {
				const inv = await findInvoiceByLegacyBillId(
					bills[0].id,
					service as any
				)
				if (inv) {
					invoice = inv
					try {
						invoiceItems = await listInvoiceItems(
							inv.id,
							service as any
						)
					} catch (_) {}
					const display =
						inv.series && inv.number != null
					...
					if (creditNotes && creditNotes.length > 0) {
						for (const cn of creditNotes as any[]) {
							...
							if (cn.pdf_url) {
								const keyCN = (cn.pdf_url as string).replace(
									/^invoices\//,
									''
								)
